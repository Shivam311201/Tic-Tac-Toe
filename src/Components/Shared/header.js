...
function Header(Myprops) {
  ...
  const customStyle1 = {
    height: my_width<454?"20%":"15%",
    width: my_width<454?"20%":"15%",
    marginLeft: "20px",
    marginRight:"10px",
...
return (
    <div>
      ...
      <Navbar.Brand href="/" style={my_width>=454?{maxWidth:"300px",color:"white"}:{maxWidth:"200px",color:"white"}}>
      <img
        ...
        alt=""
      />{' '}
      {my_width>=454?<h1 class="heading m-0">Tic-Tac-Toe</h1>:<h2 class="heading m-0">Tic-Tac-Toe</h2>}
    </Navbar.Brand>
        <div style={{display: "inline-block", verticalAlign: "middle" }}>
        {(my_width>=780)&&(<div style={{ display: "inline-block", verticalAlign: "middle" }}>
        <Link
              to="/"
              style={
                isHovered[0] === true
          ...
          <button
            ...
            onMouseEnter={() => setHovered([true, false])}
            onMouseLeave={() => setHovered([false, false])}
          >
          Home <HomeIcon />            
          </button>
          </Link>
          ...
          {my_width<780&&<Dashboard value={Myprops.value} checkLoad={Load_func}/>}
        </div>
      </nav>
    </div>
  );
